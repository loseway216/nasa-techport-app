<template>
  <div class="bg-stone-300 shadow rounded p-2">
    <div class="flex mb-2">
      <NuxtLink class="font-bold text-sky-800 hover:text-sky-600 hover:underline text-lg truncate max-w-2xl"
        :to="`/details/${project.projectId}`">{{ project.title }}</NuxtLink>
      <span class="text-white rounded shadow px-2 ml-auto" :class="statusClass">{{ project.statusDescription }}</span>
    </div>
    <div class="flex space-y-2 sm:space-y-0 space-x-0 sm:space-x-4 flex-wrap sm:flex-nowrap">
      ...
      <div class="flex flex-col flex-1 overflow-hidden basis-1/2 sm:basis-auto">
      ...
      <div class="flex flex-col flex-1 overflow-hidden basis-1/4 sm:basis-auto">
